return {
  ...
  profile = {
    records = {
      ...
      archive1 = "干员蓝毒认为自己在战场上的工作是足够骇人但不可缺少的，她努力适应着罗德岛对她给予的种种行动上的需求，并总能回报以优良绩效。<br>在行动过程中，她会以充满个人风格的幽默来消除战斗带来的压力，并缓解同行干员对她的提防（例如在战斗前为大家配发解毒的药剂）。<br>蓝毒所使用的小型手弩被证明是一种拥有极为优秀的射程且威力不俗的狙击武器，而她自行调配的含有毒制剂的弩箭更使她的每一次攻击都会具备致命的杀伤力。蓝毒从不会让敌人以凄惨的状态死去，因为这实在不怎么优雅。",
